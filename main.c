...
UserArgs* my_env(char* line)
{
   ...
   uargs = (UserArgs*)malloc(sizeof(UserArgs*));
   uargs->argc = 0;


   for (i = 0; i < numWords; i++)
   {
	uargs->argv[i] = getWord(line, i);
	uargs->argc += 1;
   }

   uargs->argv[numWords] = NULL;
...
/*
	if a userargs string contains the input/output/pipe/&
...
int my_execute(UserArgs* uargs)
{
   ...
   if (strcmp(uargs->argv[0], "exit")==0)
   {
	...
	/*if we need to check if directory exists, check below value for 
access*/
        setenv("PWD", path, 0);
